...
public class UserInfoController {
					...
					.setText( // 메일 내용
							new StringBuffer()
									.append("메일인증 \n").append("Arisol에 가입해주셔서 감사합니다. \n" + serverAddress
											+ "users/verify?userid=" + joinUser.getUserid())
									.append("\n이메일 인증 확인").toString());
			try {
	 ...
	 * @comment : 회원 로그인 처리
	 * 
	 * @param loginInfo : View에서 입력받은 User ID와 User Pw를 저장하고 있는 객체 session : 로그인
	 * 성공 시 User ID를 저장 model : 로그인 실패 시 실패 메시지를 User에게 알리기 위한 메시지 저장
	 * 
	 * @author : 정보승
	 */
	...
	public String userLogin(UserInfoVO loginInfo, HttpSession session, Model model) {
		...
		char userVerified = (user != null) ? user.getEmailverify() : 'N'; // E-mail
																			// 인증
																			// 여부
		...
		if (user != null && userVerified == 'Y') {
			...
			session.setAttribute("userid", user.getUserid()); // 로그인 성공시 User
																// ID를 Session에
																// 저장
