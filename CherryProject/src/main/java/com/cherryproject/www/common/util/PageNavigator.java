...
public class PageNavigator {
	//페이지 관련 정보 
	private int countPerPage;		//페이지당 글목록 수, 하나의 페이지에 글을 몇개 보여줄 것인가
 	private int pagePerGroup;		//그룹당 페이지 수, 하나의 페이지에 몇개의 페이지 목록을 보여줄것인가(ex)5일경우, 1 2 3 4 5)
	private int currentPage;		//현재 페이지 (최근 글이 1부터 시작),
	private int totalRecordsCount;	//전체 글 수
	private int totalPageCount;		//전체 페이지 수
	private int currentGroup;		//현재 그룹 (최근 그룹이 0부터 시작), (0번째 그룹이면   1 2 3 4 5  1번째 그룹 6 7 8 9 10)
	private int startPageGroup;		//현재 그룹의 첫 페이지 (0번째 그룹이면 1, 1번째 그룹 6)
	private int endPageGroup;		//현재 그룹의 마지막 페이지 (0번째 그룹이면 5, 1번째 그룹 10)
	private int startRecord;		//전체 레코드(전체글) 중 현재 페이지(현재보여지는페이지) 첫 글의 위치 (0부터 시작), 어디에서부터 셀렉트를 해야하는지 
